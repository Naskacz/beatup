...
Game::Game() :window(sf::VideoMode(800, 600), "BeatUp") {
	...
	if (!backgroundTexture.loadFromFile("images\\backgroundGame.png")) {
...
std::string fileChoice(const std::string& podfolder, std::string& sciezka) {
	std::filesystem::path folder = std::filesystem::current_path();
	if (!std::filesystem::exists(folder)) {
		std::cerr << "Folder nie istnieje: " << folder << "\n";
		return "";
	}
	std::string fullPath = folder.string() + "\\" + podfolder + "\\";
	sciezka = tinyfd_openFileDialog(
		"Wybierz plik",
		fullPath.c_str(),
		0, nullptr, nullptr, 0);
	if (!sciezka.empty()) {
...
void Game::beatmapFileChoice() {
	beatmapFile = fileChoice("beatmaps", beatmapFile);
}
void Game::songFileChoice() {
	songFile = fileChoice("songs", beatmapFile);
}
void Game::run() {
	while (window.isOpen()) {
		processEvents();
	...
	while (window.pollEvent(event)) {
		...
		if (state == GameState::Menu && event.type == sf::Event::MouseButtonPressed) {
			...
			if (choice == 0) {
				...
				noteManager.clearNotes();
				noteManager.getBeatmap().clearNotes();
				songFileChoice();
				if (!music.openFromFile(songFile)) {
...
void Game::update(sf::Time) {
	if (state == GameState::Playing) {
		...
		if (music.getStatus() == sf::SoundSource::Status::Stopped) {
			std::string filenameBeatmapFile = std::filesystem::path(beatmapFile).filename().string();
			Score s{
			...
			std::string resp;
			/*std::cout << "{\"nickname\":\"" << s.nickname << "\",\"score\":" << s.score << ",\"beatmap\":\"" << s.beatmap<< "\"}" << std::endl;*/
			...
			std::string beatmapName = s.beatmap;
			httpClient.fetchScores(beatmapName, topScores);
