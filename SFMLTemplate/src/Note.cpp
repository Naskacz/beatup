...
Note::Note(float X, float Y, float targetTime) : targetTime(targetTime),
appearTime(targetTime - 1.0f), ringRadius(80.f), ringThickness(4.f) {
...
bool Note::isReadyToClick() const {
	...
	return elapsed >= (targetTime - appearTime-0.5f);
}